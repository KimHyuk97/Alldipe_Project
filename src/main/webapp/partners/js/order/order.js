function deliveryStart(){
	...
	for(var i = 0; i<selected.length; i++){
		if(selected[i].checked && orderStatus[i].innerText != "상품준비중"){
			alert('상품 준비중 상태의 주문만 수정 가능합니다.');
			return false;
		}
	}
	if(confirm("해당 주문을 발송처리 하시겠습니까?")){
		document.getElementById("changeStatus").value = "배송중";
		order.submit();
	}
}
...
function orderCheck(){
	...
	for(var i = 0; i<selected.length; i++){
		if(selected[i].checked && orderStatus[i].innerText != "주문완료"){
			alert('주문 완료 상태의 주문만 수정 가능합니다.');
			return false;
		}
	}
	if(confirm("해당 주문 상태를 상품 준비중으로 변경하시겠습니까?")){
		document.getElementById("changeStatus").value = "상품준비중";
		order.submit();
	}
}
...
function selectItem(){
	...
	const selected = document.getElementsByClassName("select_item");
	const selectAll = document.getElementById("selectAll");
	for(var i=0; i<selected.length; i++){
		if(selectAll.checked && !selected[i].disabled){
