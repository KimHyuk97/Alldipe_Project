...
<div id="modal"></div>
<div class="cont_wrap">	
 	<form action="memberListWindowMode" method="post" name="list">
 		<input type="hidden" name="mode" value="<%=mode%>">
        <div class="search_cont">
            <div class="sch_tit">
                <h2>회원 리스트</h2> 
				<button type="button" onclick="payMode('<%=mode%>')" id="flxed">지급/차감</button>    
            </div>
            <div class="sch_ck_cont">
                ...
                <div class="sch_check_btn"><button type="submit">검색</button></div>
            </div>     
        </div>
        ...
        <div style="margin: 30px 0; text-align: center;">
        	<%if(paging.getPage() > 1){ %>        	
				<a href="memberListWindowMode?mode=mileage&page=${paging.page-1}&kind=<%=kind %>&keyword=<%=keyword %>&apply=<%=apply %>&minDay=<%=minDay %>&maxDay=<%=maxDay %>&minMileage=<%=minMileage %>&maxMileage=<%=maxMileage %>&gender=<%=gender %>&sort=<%=sort %>&limit=<%=limit %>">[이전]</a>&nbsp;
        	<%}%>
			<c:forEach begin="${paging.startPage}" end="${paging.endPage }" var="i" step="1">
				<c:choose>
					...
					<c:otherwise>
						<a href="memberListWindowMode?mode=mileage&page=${i}&kind=<%=kind %>&keyword=<%=keyword %>&apply=<%=apply %>&minDay=<%=minDay %>&maxDay=<%=maxDay %>&minMileage=<%=minMileage %>&maxMileage=<%=maxMileage %>&gender=<%=gender %>&sort=<%=sort %>&limit=<%=limit %>">${i}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<%if(paging.getPage() < paging.getMaxPage()){ %>
				<a href="memberListWindowMode?mode=mileage&page=${paging.page+1}&kind=<%=kind %>&keyword=<%=keyword %>&apply=<%=apply %>&minDay=<%=minDay %>&maxDay=<%=maxDay %>&minMileage=<%=minMileage %>&maxMileage=<%=maxMileage %>&gender=<%=gender %>&sort=<%=sort %>&limit=<%=limit %>">&nbsp;[다음]</a>			
			<%} %>
		</div>
	</form>
   </div>
