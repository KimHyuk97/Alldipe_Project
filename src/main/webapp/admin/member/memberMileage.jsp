...
<%
	...
	ArrayList<membermileageDTO> list = (ArrayList)request.getAttribute("list");
...
<div class="cont_wrap">
	<form action="./memberMileage" method="post" name="form">
            ...
            <div class="sch_ck_cont">
                ...
                <div class="sch_check">
                    ...
                    <label><input type="radio" name="handleMode" value="적립" <%if(handleMode != null && handleMode.equals("적립")){  %> checked = "checked" <%} %>>적립</label>
                </div>
                <div class="sch_check">
                    <p>금액범위</p>
        ...
        <div class="sch_result">
            ...
            <div class="sch_btn">
                <div class="btn_left">
 					...
 					<button type="button" onclick="payTheMileage()">마일리지 지급</button>
                </div>
                <div class="btn_right">
            ...
            <table>
                ...
                <tr>
                    ...
                    <td><%=df.format(list.get(i).getBeforeMileage()) %></td>
                    <td><%=df.format(list.get(i).getAfterMileage())%></td>
        ...
        <div style="margin: 30px 0; text-align: center;">
        	<%if(paging.getPage() > 1){ %>        	
				<a href="memberMileage?page=${paging.page-1}&kind=<%=kind %>&keyword=<%=keyword %>&handleMode=<%=handleMode %>&minDay=<%=date01 %>&maxDay=<%=date02 %>&minMileage=<%=price01 %>&maxMileage=<%=price02 %>&sort=<%=sort %>&limit=<%=limit %>">[이전]</a>&nbsp;
        	<%}%>
			<c:forEach begin="${paging.startPage}" end="${paging.endPage }" var="i" step="1">
				<c:choose>
					...
					<c:otherwise>
						<a href="memberMileage?page=${i}&kind=<%=kind %>&keyword=<%=keyword %>&handleMode=<%=handleMode %>&minDay=<%=date01 %>&maxDay=<%=date02 %>&minMileage=<%=price01 %>&maxMileage=<%=price02 %>&sort=<%=sort %>&limit=<%=limit %>">${i}</a>
					</c:otherwise>
				</c:choose>
			</c:forEach>
			<%if(paging.getPage() < paging.getMaxPage()){ %>
				<a href="memberMileage?page=${paging.page+1}&kind=<%=kind %>&keyword=<%=keyword %>&handleMode=<%=handleMode %>&minDay=<%=date01 %>&maxDay=<%=date02 %>&minMileage=<%=price01 %>&maxMileage=<%=price02 %>&sort=<%=sort %>&limit=<%=limit %>">&nbsp;[다음]</a>			
			<%} %>
		</div>
	</form>
</div>
...
<script src="${pageContext.request.contextPath}/admin/js/member/mileage.js"></script>