...
/**
 * Servlet implementation class adminThemeClassifyModController
 */
@WebServlet("/admin/goods/themeClassifyMod")
public class adminThemeClassifyModController extends HttpServlet {
	...
	protected void doProcess(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(list.size()>0){
			...
			dto.setSno(Integer.parseInt((String)list.get("themeSno")));
			...
			dto.setGoodsNos(((String)list.get("recomGoodsNo")).equals("")?"":((String)list.get("recomGoodsNo")));
			//	이미지 파일 설정
			if(filelist.size()>0){
			...
			if(chk>0){
				out.println("alert('테마("+dto.getThemeNm().toString()+")를 수정하였습니다!');");
